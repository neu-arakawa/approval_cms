    ...
    <div>
        ...
        <div class="publish_term">
            <div class="form-row align-items-center align-top">
                ...
                <div class="col-md-10">
                    <div class="col-md-12">
                        <label class="">
                        <input name="publish_term" type="radio" value="2" <?= set_value('publish_term','2') == '2' ? 'checked=checked' :'' ?>> 表示日付より1年間
                        </label>
                    </div>
                    <div class="col-md-12">
                        <label class="">
                            <input name="publish_term" type="radio" value="0" <?= set_value('publish_term') == '0' ? 'checked=checked' :'' ?>> 指定しない
                        </label>
                    </div>
                    <div class="col-md-12">
                        <div class="form-row align-items-center">
                            <label class="radio col-auto">
                                <input name="publish_term" type="radio" value="1" <?= set_value('publish_term') == '1' ? 'checked=checked' :'' ?>> 指定する
                            </label>
                            <?php
        ...
        <?php else: // 確認画面?>
            ...
            <?php if (!empty($_POST['flg_publish'])):?>
                ...
                if (set_value('publish_term') == '1' && !empty(array_filter($d))){
                    echo implode(' 〜 ', $d);
                }
                else if( set_value('publish_term') == '2'){
                    echo '表示日付より1年間';
                }
                else{
                    echo '指定しない';
...
$(document).ready(function(){
    ...
    // if (term_is_set) { // 期間設定あり
        // $term_radio.filter('[value="1"]').prop('checked',true);
    // } else { // 期間設定なし
        // $term_radio.filter('[value="2"]').prop('checked',true);
    // }
    _refresh_publish();
